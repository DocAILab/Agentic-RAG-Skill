"""Manifest creation and local TriviaQA subset integrity checks."""
...
class DatasetStateError(RuntimeError):
    """Raised when a local TriviaQA subset is incomplete or inconsistent."""
...
def file_record(path: Path, root: Path) -> dict:
    return {
        ...
        "records": subset_samples(path),
        "bytes": path.stat().st_size,
        "sha256": sha256_file(path),
    }


def subset_samples(path: Path) -> int:
    """Return the sample count recorded in a TriviaQA subset JSON file."""
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    samples = payload.get("samples")
    if not isinstance(samples, list):
        raise ValueError(f"subset file has no samples list: {path}")
    return len(samples)
...
def _validate_file_record(root: Path, record: dict) -> None:
    ...
    if subset_samples(path) != record["records"]:
